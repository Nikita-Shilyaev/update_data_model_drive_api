"""Загрузка инкрементов akzii с FTP-сервера на Google Drive за диапазон дат.
...
FTP_DIR = config["FTP"]["akzii_dir"]

CREDS_PATH = PROJECT_DIR / config["DRIVE_API"]["creds"]
TOKEN_PATH = PROJECT_DIR / config["DRIVE_API"]["token"]

DRIVE_FOLDER_ID = config["PIPELINE_RAW"]["pipeline_raw_akzii_id"]
PIPELINE_STATE_ID = config["PIPELINE_STATE"]["pipeline_state_id"]


def parse_args():
    parser = argparse.ArgumentParser(
        description="Загрузка инкрементов akzii с FTP на Google Drive за диапазон дат"
    )
    parser.add_argument(
        "--date",
        dest="ds_run",
