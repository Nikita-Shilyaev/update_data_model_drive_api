"""Обновление Vozvraty_target.csv инкрементами из raw/vozvraty на Google Drive.

Инкременты по возвратам — чистый append: каждый день на FTP выкладывается файл
с новыми возвратами, они уже лежат в raw/vozvraty/<дата>/. Скрипт обрабатывает
...
Дубликаты снимаются по паре (`штрихкод`, `возврат`) с приоритетом более свежей
...
# читаем номера строками, иначе пустые значения нельзя однозначно отличить
# от валидных числовых номеров при проверке на пропуски
INCREMENT_DTYPES = {
    "Возврат товаров от покупателя.Номер": str,
    "Чек.Номер": str,
}

KEY_COLUMNS = ["штрихкод", "возврат"]
TARGET_COLUMNS = [
    "штрихкод",
    "возврат",
    "чек",
...
RAW_VOZVRATY_ID = config["PIPELINE_RAW"]["pipeline_raw_vozvraty_id"]
VOZVRATY_TARGET_ID = config["PIPELINE_PROCESSED"]["vozvraty_target_id"]
...
def find_increment_files(service, date_strs):
    """Ищет в raw/vozvraty эксель-файл для каждой даты из date_strs.
    ...
    date_folders = {
        item["name"]: item["id"]
        for item in list_children(service, RAW_VOZVRATY_ID)
    ...
    if missing:
        logger.error(
            "В raw/vozvraty нет инкрементов за даты: %s. "
...
def transform_increment(raw):
    """Приводит инкремент vozvraty к схеме целевого файла.

    Если за дату не было возвратов, выгрузка содержит только заголовки и одну
    строку со словом "Итого" в первом столбце — такую строку отбрасываем явно,
    до попытки привести числовые колонки к нужным типам. Такая же строка
    "Итого" может завершать и обычный, непустой инкремент.
    """
    if raw.empty:
    ...
    # "Возврат товаров от покупателя.Номер" — обязательное поле, пустые удаляем
    num_column = "Возврат товаров от покупателя.Номер"
    incr = raw.dropna(subset=[num_column]).copy()
    incr[num_column] = incr[num_column].str.strip()
    incr = incr[incr[num_column] != ""].copy()
    ...
    voz_date = pd.to_datetime(
        incr["Возврат товаров от покупателя.Дата"], dayfirst=True, errors="coerce"
    )
    chek_date = pd.to_datetime(incr["Чек.Дата"], dayfirst=True, errors="coerce")

    # поле "возврат" собираем по аналогии с полем "чек" в sales_cheki
    incr["возврат"] = (
        "Возврат " + incr[num_column] + " от " + voz_date.dt.strftime("%d.%m.%Y %H:%M:%S")
    )

    # поле "чек" — связь с sales_cheki; заполнено только там, где возврат
    # привязан к продаже, иначе остаётся пустым
    chek = "Чек " + incr["Чек.Номер"] + " от " + chek_date.dt.strftime("%d.%m.%Y %H:%M:%S")
    incr["чек"] = chek.where(incr["Чек.Номер"].notna() & chek_date.notna())

    incr["дата"] = voz_date.dt.strftime("%Y-%m-%d %H:%M:%S")
    incr["date"] = voz_date.dt.strftime("%Y-%m-%d")

    incr["магазин"] = (
        incr["Возврат товаров от покупателя.Магазин.Наименование"]
        .str.strip()
    ...
    incr["телефон"] = incr[
        "Возврат товаров от покупателя.Дисконтная карта покупателя.Код карты"
    ].apply(phone_processor)

    incr = incr.rename(
        columns={
            "Продавец.ФИО": "продавец",
            "Возврат товаров от покупателя.Дисконтная карта покупателя.Код": "код",
...
def update_vozvraty(ds_run=None):
    """Дописывает инкременты vozvraty из raw/vozvraty в Vozvraty_target.csv.
    ...
    if raw_updated is None:
        logger.error(
            ...
            "source=%s, layer=%s. Сначала запустите raw/load_vozvraty_increment.py.",
            SOURCE,
            RAW_LAYER,
        )
        sys.exit(1)
    ...
    if start_date > end_date:
        logger.info(
            "Нет новых дат для обработки vozvraty: последнее обновление %s, "
    ...
    logger.info(
        "Диапазон обработки vozvraty: с %s по %s (%s дн.)",
        date_strs[0],
        date_strs[-1],
        len(date_strs),
    )
    ...
    for ds in date_strs:
        ...
        logger.info("Инкремент vozvraty за %s: строк после обработки %s", ds, len(incr))

    # 3. Читаем базовый файл
    target = pd.read_csv(
        download_bytes(service, VOZVRATY_TARGET_ID),
        dtype={"штрихкод": str, "возврат": str, "чек": str, "код": str, "телефон": str},
    )
    logger.info("Базовый файл Vozvraty_target.csv: строк %s", len(target))

    missing_columns = [column for column in TARGET_COLUMNS if column not in target.columns]
    if missing_columns:
        logger.error(
            "В Vozvraty_target.csv нет колонок: %s. "
    ...
    update_file_bytes(service, VOZVRATY_TARGET_ID, out.getvalue(), "text/csv")
    logger.info(
        "Vozvraty_target.csv перезаписан: строк всего %s (добавлено %s)",
        len(combined),
    ...
    logger.info(
        "Готово: обновление vozvraty за %s … %s выполнено", date_strs[0], date_strs[-1]
    )


def parse_args():
    parser = argparse.ArgumentParser(
        description="Обновление Vozvraty_target.csv инкрементами из raw/vozvraty на Google Drive"
    )
    parser.add_argument(
        "--date",
        dest="ds_run",
