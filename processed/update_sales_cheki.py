...
В выгрузке вместе с продажами приходят возвратные чеки — их различает поле
"Чек.Вид операции" (`Продажа` / `Возврат`). 1С отдаёт количество и суммы
возврата положительными (строка "Итого" складывает их с плюсом), поэтому у
возвратов `количество`, `сумма_скидки_оплаты_бонусом`, `сумма_со_скидкой` и
`сумма_без_скидки` умножаются на −1: тогда накопительные суммы по
клиенту/магазину считаются простым сложением.

Строки одного инкремента с одинаковой парой (`штрихкод`, `чек`) — несколько
изделий одной номенклатуры в чеке — схлопываются в одну с суммированием
количества и сумм. Дубликаты между инкрементами и базой снимаются по той же
паре с приоритетом более свежей строки: инкременты присоединяются в
хронологическом порядке после базы, поэтому `keep="last"` оставляет последнюю
известную версию строки. Это же даёт идемпотентность — повторная обработка той
же даты не удвоит строки.
...
OPERATION_RETURN = "Возврат"

# аддитивные поля строки чека: у возвратов инвертируются, чтобы накопления
# считались сложением; при схлопывании строк одного ключа — суммируются
AMOUNT_COLUMNS = [
    "количество",
    "сумма_скидки_оплаты_бонусом",
    "сумма_со_скидкой",
...
def transform_increment(raw):
    ...
    incr["магазин"] = incr["Чек.Магазин.Наименование"].replace(STORE_MAP)
    ...
    incr["вид_операции"] = incr["Чек.Вид операции"].astype("string").str.strip()
    ...
    is_return = incr["вид_операции"].eq(OPERATION_RETURN).fillna(False)
    incr.loc[is_return, AMOUNT_COLUMNS] = -incr.loc[is_return, AMOUNT_COLUMNS]

    # Один штрихкод может стоять в чеке несколькими строками (несколько изделий
    # одной номенклатуры), и 1С не всегда схлопывает их в одну с количеством > 1.
    # Дедуп по ключу оставил бы только одну такую строку, поэтому заранее
    # собираем их в одну: суммы складываем, остальные поля берём из первой
    # строки (это атрибуты чека, одинаковые у всех его строк; продавец в 1С
    # изредка различается по строкам — берём первого).
    # min_count=1 не превращает пустой бонус в 0.
    aggregations = {
        column: "first" for column in TARGET_COLUMNS
        if column not in KEY_COLUMNS and column not in AMOUNT_COLUMNS
    }
    aggregations.update({column: lambda s: s.sum(min_count=1) for column in AMOUNT_COLUMNS})
    incr = incr.groupby(KEY_COLUMNS, as_index=False, sort=False).agg(aggregations)
