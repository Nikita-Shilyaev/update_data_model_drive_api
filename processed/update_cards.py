...
Дубликаты снимаются по `телефону`: при миграции карт в новую нумерацию
(`Kaz...`) один и тот же человек мог получить второй код на тот же телефон —
это не два разных клиента, а одна карта под новым кодом. Из дублей остаётся
запись с самой поздней `дата_открытия_карты` (отсутствующая дата считается
самой ранней), что и оставляет актуальную карту. Внутри одинаковой даты
(включая повторную обработку той же даты) порядок дублей стабилен и совпадает
с порядком присоединения (база → инкременты по датам), так что `keep="last"`
на равных датах ведёт себя как раньше — это даёт идемпотентность: повторная
обработка того же дня не удваивает строки.
...
def transform_increment(raw):
    ...
    incr["магазин_открытия_карты"] = incr["магазин_открытия_карты"].replace(MAP_STORE)
...
def update_cards(ds_run=None):
    ...
    combined["_дата_открытия_сорт"] = pd.to_datetime(
        combined["дата_открытия_карты"], errors="coerce"
    )

    rows_before = len(combined)
    combined = combined.sort_values(
        ["_дата_открытия_сорт", "_orig_order"], na_position="first", kind="stable"
    )
    combined = combined.drop_duplicates(subset=KEY_COLUMNS, keep="last")
    combined = (
        combined.sort_values("_orig_order")
        .drop(columns=["_дата_открытия_сорт", "_orig_order"])
