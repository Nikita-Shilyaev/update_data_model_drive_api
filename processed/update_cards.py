...
Дубликаты снимаются только по `телефону`: при миграции карт в новую
нумерацию (`Kaz...`) один и тот же человек мог получить второй код на тот же
телефон — это не два разных клиента, а одна карта под новым кодом. Из дублей
остаётся та запись, что пришла позже (`keep="last"` в порядке присоединения:
база → инкременты по датам), — это и актуальная карта, и идемпотентность:
повторная обработка того же дня не удваивает строки.

`магазин_первой_покупки` заполняется из `магазин_открытия_карты`: карту
открывают при покупке, поэтому магазин открытия и есть магазин первой покупки.
...
def transform_increment(raw):
    ...
    # карту открывают при покупке — магазин открытия и есть магазин первой
    # покупки; vip проставляется по справочнику уже после объединения с базой
    incr["магазин_первой_покупки"] = incr["магазин_открытия_карты"]
...
def update_cards(ds_run=None):
    ...
    # 4. Присоединяем инкременты и снимаем дубликаты по телефону — пришедшая
    # позже строка вытесняет старую (см. докстринг модуля)
    combined = pd.concat([target, *increments], ignore_index=True)

    rows_before = len(combined)
    combined = combined.drop_duplicates(subset=KEY_COLUMNS, keep="last").reset_index(
        drop=True
    )
    logger.info(
